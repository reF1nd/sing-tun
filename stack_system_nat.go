...
func (n *TCPNat) checkTimeout() {
	...
	for _, e := range expired {
		n.removeExpiredSessionLocked(e.port, e.session, now)
	}
	n.portAccess.Unlock()
	n.addrAccess.Unlock()
}

// removeExpiredSessionLocked rechecks a timeout snapshot while both map locks are held.
func (n *TCPNat) removeExpiredSessionLocked(port uint16, session *TCPSession, now time.Time) {
	// Allocation may have reused this port since the timeout scan released its read lock.
